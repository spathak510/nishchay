...
def idfc_digitization(pdf_path, pdf_password):
    ...
    try:
        ...
        tables = tabula.read_pdf(pdf_path, pages='all', lattice=True, password=passcode)
        info = tabula.read_pdf(pdf_path, pages='1', password=passcode,area=[80,46,579,535],stream=True,guess=False,pandas_options={'header':None})
    except:
