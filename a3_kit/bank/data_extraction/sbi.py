...
def sbi_digitization(pdf_path,pdf_password):
    ...
    try:
        tables = tabula.read_pdf(pdf_path, pages='all', password=passwrd, pandas_options = col2str)
    except:
    ...
    if "Date (Value Date)" in tables[0].columns:
        ...
        tables=tabula.read_pdf(pdf_path, pages='all',stream=True, password=passwrd,pandas_options={'header':None,'dtype': str})
        ...
        #accont information
        cust_info=tabula.read_pdf(pdf_path, pages=1,stream=True, password=passwrd,area=[97.6,27.8,479.9,583.8], pandas_options = col2str)
